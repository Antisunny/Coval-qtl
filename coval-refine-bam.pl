...
if ($stderr1 =~ /text/){
    ...
    if ($long_read == 0){
	    my $ave_read_length = &calc_readlen($input_file, 'sam');
    	if ($ave_read_length == 0){
    	    die "Input file cannot be opened\n";
    	}
    	print STDERR "Average read length: $ave_read_length bp\n";
    	if ($ave_read_length > 151){
    	    $long_read = 1;
    	    if ($ave_read_length > 350){
    		    print STDERR "### Warrant: The read length appears to exceed 350 bp. Coval-Refine guarantees only reads with <= 300 bp. ###\n\n";
    	    }
    	}
    }
    if ($out_sam == 1){
        if ($read_type eq 'PE'){
	    my $new_arg = $arg;
	    if ($ave_insert_size_SD eq 'auto'){
		$new_arg = &stat_ins_size('sam');
	    }
	    if ($long_read == 0){
		...
		system ("$com_path/coval-refine-sam-LR.pl $new_arg $input_file > $out_prefix.sam") if ($out_prefix ne '');
		system ("$com_path/coval-refine-sam-LR.pl $new_arg $input_file") if ($out_prefix eq '');
	    }
        }
        else{
	    if ($long_read == 0){
		...
		system ("$com_path/coval-refine-sam-LR.pl $arg $input_file > $out_prefix.sam") if ($out_prefix ne '');
		system ("$com_path/coval-refine-sam-LR.pl $arg $input_file") if ($out_prefix eq '');
	    }
        }
    }
    else{
      ...
      #            system ("$com_path/coval-refine-sam.pl $new_arg $input_file | samtools view -uSt $reference_file.fai - 2>/dev/null | samtools sort - $out_prefix 2>/dev/null");
	    system ("$com_path/coval-refine-sam.pl $new_arg $input_file | samtools view -bSt $reference_file.fai - > $out_prefix.bam") if ($long_read == 0);
	    system ("$com_path/coval-refine-sam-LR.pl $new_arg $input_file | samtools view -bSt $reference_file.fai - > $out_prefix.bam") if ($long_read == 1);
        }
        else{
            system ("$com_path/coval-refine-sam.pl $arg $input_file | samtools view -bSt $reference_file.fai -  > $out_prefix.bam") if ($long_read == 0);
	    system ("$com_path/coval-refine-sam-LR.pl $arg $input_file | samtools view -bSt $reference_file.fai -  > $out_prefix.bam") if ($long_read == 0);
        }
    }
}
else{
    ...
    my $ave_read_length = &calc_readlen($input_file, 'bam');
    if ($ave_read_length == 0){
	die "Input file cannot be opened\n";
    }
    print STDERR "Average read length: $ave_read_length bp\n";
    ...
    if ($out_sam == 1){
		...
		system ("samtools view $input_file | $com_path/coval-refine-sam-LR.pl $new_arg - > $out_prefix.sam") if ($out_prefix ne '');
		system ("samtools view $input_file | $com_path/coval-refine-sam-LR.pl $new_arg -") if ($out_prefix eq '');
	    }
        }
        else{
	    if ($long_read == 0){
		...
		system ("samtools view $input_file | $com_path/coval-refine-sam-LR.pl $arg - > $out_prefix.sam") if ($out_prefix ne '');
		system ("samtools view $input_file | $com_path/coval-refine-sam-LR.pl $arg -") if ($out_prefix eq '');
	    }
        }
    }
    else{
      ...
      #            system ("samtools view $input_file | $com_path/coval-refine-sam.pl $new_arg - | samtools view -uSt $reference_file.fai - 2>/dev/null | samtools sort - $out_prefix 2>/dev/null");
	    system ("samtools view $input_file | $com_path/coval-refine-sam.pl $new_arg - | samtools view -bSt $reference_file.fai - > $out_prefix.bam") if ($long_read == 0);
	    system ("samtools view $input_file | $com_path/coval-refine-sam-LR.pl $new_arg - | samtools view -bSt $reference_file.fai - > $out_prefix.bam") if ($long_read == 1);
        }
        else{
            system ("samtools view $input_file | $com_path/coval-refine-sam.pl $arg - | samtools view -bSt $reference_file.fai - > $out_prefix.bam") if ($long_read == 0);
	    system ("samtools view $input_file | $com_path/coval-refine-sam-LR.pl $arg - | samtools view -bSt $reference_file.fai - > $out_prefix.bam") if ($long_read == 1);
...
sub calc_readlen {
    ...
    open (FILE, "samtools view $file |") or die "$file is not found: $!\n" if ($tag eq 'bam');
    while (my $line = <FILE>){
    	if ($line =~ /^\@/){
    	    next;
    	}
    ...
    close (FILE);
    my $ave_len = int ($sum_len / $count + 0.5);
    return $ave_len;
}
