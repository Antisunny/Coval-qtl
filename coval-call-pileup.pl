...
covar call [options] <input_pileup_file>
  ...
  Options:
   ...
   --maxr or -m <INT>       maximum read number covering non-reference allele [default: auto]
...
open (FILE, $input_file) or die "$input_file: $!";
	...
	$solexa_type += $base_qual =~ /LMNOPQRSTUVWXYZ\[\\\]\^_`abcdefgh/g;
        last if ($sanger_type >= 10) or ($solexa_type >= 10);
    }
...
if ($maximum_read_depth == 0){
    ...
    close (FILE);
    my $ave_depth = int ($sum_depth / $allele_num * 10) / 10;
    $maximum_read_depth = $ave_depth * 3;
    print STDERR "Average read depth: $ave_depth\n";
    print STDERR "Maximum read number specified: $maximum_read_depth\n";
}
...
#        next if ($ref_base !~ /[ACGTacgt\*]/);
		...
		my $alt_freq = int ($line[10] / $read_depth * 100) / 100;
		$line[15] = $alt_freq;
		    ...
		    my $alt_freq = int ($line[11] / $read_depth * 100) / 100;
		    $line[15] = $alt_freq;
                    my $new_line = join ("\t", @line);
                    push (@indel_lines, $new_line);
                }
