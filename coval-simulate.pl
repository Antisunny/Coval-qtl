...
coval simulate [options] -r <reference file> -p <prefix of output files>
  ...
  Options:
   ...
   --snp or -s <FLOAT>      genomic mutation rate for SNPs [default: 0.001]
   --indel or -i <FLOAT>    genomic mutation rate for indels [default: 0.0001]
...
open (FILE2, "> $out_indel");
print FILE2 "Chr\tPos\tVar\tRef\tAlt\tRef-pos\n";
foreach (@indel_info){
	print FILE2 $_, "\n";	
}
close (FILE2);
...
#print STDERR $pos, "\t", $indel, "\t", $indel_base, "\t", $ref_1st_pos, "\t", $ref_1st_pos_2, "\t", $sum_frac_adjust_size, "\n";
					...
					my $new_line = $chr . "\t" . $pos . "\t" . 'S' . "\t" . $ref_base . "\t" . $snp_base . "\t" . $orig_pos;
			...
			foreach my $key (sort keys %chr_seq){
				...
				if (($ref_1st_pos_2 <= $pos) and ($ref_1st_pos_2 + length ($chr_seq{$key}) - 1 > $pos) and ($chr eq $ref_chr)){
					...
					my $new_line = $chr . "\t" . $pos . "\t" . 'D' . "\t" . '-' . "\t" . $ref_base . "\t" . $orig_pos;
					push (@indel_info, $new_line);
			...
			foreach my $key (sort keys %chr_seq){
				...
				if (($ref_1st_pos_2 <= $pos) and ($ref_1st_pos_2 + length ($chr_seq{$key}) - 1 > $pos) and ($chr eq $ref_chr)){
					...
					my $new_line = $chr . "\t" . $pos . "\t" . 'I' . "\t" . '+' . "\t" . $seq . "\t" . $orig_pos;
