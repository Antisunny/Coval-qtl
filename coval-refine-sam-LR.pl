...
=head1 SYNOPSIS
 ...
 coval refine [options] <input_sorted_bam/sam_file> (Read names and reference sequence names in sam file should not contain characters '||' or '='.)
  ...
  Options:
   ...
   --long_read or -lr      specify if the read length is > 150 bp (Coval-refine guarantees only reads <= 300 bp) [default: auto]
...
open (FILE, $input_file) or die "$input_file: $!";
	...
	$read_number++;
	if (($cigar =~ /H/) or ($cigar =~ /^\d+S.+S$/) or ($cigar =~ /^\d+S\d+M\d+[DI]\d+M\d+[DI]/) or ($cigar =~ /\d+[DI]\d+M\d+[DI]\d+M\d+S$/) or ($cigar =~ /\d+[DI]\d+M\d+[DI]\d+M\d+[DI]\d+M\d+[DI]/)){
	    if ($filter_pairs == 1){
		$filtered_pairs{$read_id} = 1;
	    }
	    next;
	}
	if ($minimum_mapq > 0){
	...
	if ($read_number_2 <= 100){
	    $sum_read_len += $read_length;
	    $ave_read_length = int ($sum_read_len / $read_number_2 + 0.5);
	}
	...
	if ($read_type eq 'PE'){
	    $discordant_read_num ++ if (($soap_aligner == 0) and ($long_insert_read == 0) and (($sam_flag == 97) or ($sam_flag == 161) or ($sam_flag == 81) or ($sam_flag == 145) or ($sam_flag == 129) or ($sam_flag == 65) or ($sam_flag == 113) or ($sam_flag == 177)
					or ($read_dist == 0)));
	    $discordant_read_num ++ if ((($soap_aligner == 1) or ($long_insert_read == 1)) and (($sam_flag == 129) or ($sam_flag == 65) or ($sam_flag == 113) or ($sam_flag == 177) or ($read_dist == 0)));
	    $discordant_read_num ++ if ((($read_dist < -($ave_dist + $maximum_insert_SD_fold * $SD_dist)) or ($read_dist > $ave_dist + $maximum_insert_SD_fold * $SD_dist)) and ($ave_dist > 0));
		...
		if ((($read_dist < -($ave_dist + $maximum_insert_SD_fold * $SD_dist)) or ($read_dist > $ave_dist + $maximum_insert_SD_fold * $SD_dist)) and ($ave_dist > 0)){
		    if ($filter_pairs == 1){
			$filtered_pairs{$read_id} = 1;
		    }
		    next;
...
#print STDERR "$mismatch_pos\t$allele\t$top_allele\t$allele_freq\t$pre_chr\t$RG\n" if ($pre_chr =~ /chr01/);
		...
		if (($read_type eq 'PE') and ($limited_mismatch_num_pairs < $limited_mismatch_num * 2)){
		    ...
		    elsif (($match_read{$key} =~ /\t\d+M\d+[IDN]\d+M\d+[IDN]\d+M\t/) or ($match_read{$key} =~ /\t\d+S\d+M\d+[IDN]\d+M\t/) or ($match_read{$key} =~ /\t\d+M\d+[IDN]\d+M\d+S\t/)){
			$num_mismatch_pair{$id} += 2;
		    }
		    elsif ($match_read{$key} =~ /\t\d+M\d+[IDN]\d+M\d+[IDN]\d+M\d+[IDN]\d+M\t/){
			$num_mismatch_pair{$id} += 3;
		    }
		...
		if ($match_read{$key} =~ /\trealigned:-*(\d+)/){
		    ...
		    if ($change_pos != 0){
			if ($read_type eq 'PE'){
			    $change_pos_read_F{$id} = $change_pos if (($sam_flag2 == 99) or ($sam_flag2 == 163) or ($sam_flag2 == 97) or ($sam_flag2 == 161) or ($sam_flag2 == 65) or ($sam_flag2 == 129));
			    $change_pos_read_R{$id} = $change_pos if (($sam_flag2 == 83) or ($sam_flag2 == 147) or ($sam_flag2 == 81) or ($sam_flag2 == 145) or ($sam_flag2 == 113) or ($sam_flag2 == 177));
			}
		    }
		}
		...
		if ($read_type eq 'PE'){
			...
			if (($sam_flag2 == 99) or ($sam_flag2 == 163) or ($sam_flag2 == 97) or ($sam_flag2 == 161) or ($sam_flag2 == 65) or ($sam_flag2 == 129)){
			    $line[8] = $line[8] - $change_pos_read_F{$readid} + $change_pos_read_R{$readid};
			    $line[7] += $change_pos_read_R{$readid};
			}
			elsif (($sam_flag2 == 83) or ($sam_flag2 == 147) or ($sam_flag2 == 81) or ($sam_flag2 == 145) or ($sam_flag2 == 113) or ($sam_flag2 == 177)){
	...
	if ($cigar eq $read_length . 'M'){
	    my $ref_seq = substr ($chr_seq{$chrom}, $pos - 1, $read_length);
		...
		my ($total_mismatch, $lowq_mismatch, $terminal_mismatch_N, $terminal_mismatch_C, $MD) = &count_mismatch_T ($read, $ref_seq, $qual, $chrom, $pos);
	    ...
	    my $ref1 = substr ($chr_seq{$chrom}, $pos - 1, $1);
	    my $read1 = substr ($read, 0, $1);
	    my $qual1 = substr ($qual, 0, $1);
	    my $ref12 = '';
	    my $read12 = '';
	    my $qual12 = '';
	    my $indel = $2;
	    my $clip_tag = 0;
	    my $SC = '';
	    if ($chr_len - $pos - $1 + 1 - $3 >= 0){
		my $ref2 = substr ($chr_seq{$chrom}, $pos + $1 - 1, $3);
		$ref12 = $ref1 . $ref2;
		my $read2 = substr ($read, $1 + $2, $3);
		$read12 = $read1 . $read2;
		my $qual2 = substr ($qual, $1 + $2, $3);
		$qual12 = $qual1 . $qual2;
		if ($realignment eq 'ON'){
		    my $ins_pos = $pos + $1 - 1;
		    my $ins_base = substr ($read, $1, $2);
		    my $ins_info = $chrom . '==' . $ins_pos . '==' . $2 . '==I==' . $ins_base;
		    $indel_read{$ins_info} = 1;
		}
	    }
	    else{
		$ref12 = $ref1;
		...
		my ($total_mismatch, $lowq_mismatch, $terminal_mismatch_N, $terminal_mismatch_C, $MD) = &count_mismatch_T ($read12, $ref12, $qual12, $chrom, $pos);
	    ...
	    my $clip_tag = 0;
	    my $SC = '';
	    if ($chr_len - $pos - $1 - $2 + 1 - $3 >= 0){
		$ref2 = substr ($chr_seq{$chrom}, $pos + $1 + $2 - 1, $3);
		$ref12 = $ref1 . $ref2;
		$ref_del = substr ($chr_seq{$chrom}, $pos + $1 - 1, $2) if ($MDtag_sam == 0);
		$read2 = substr ($read, $1, $3);
		$read12 = $read1 . $read2;
		$qual2 = substr ($qual, $1, $3);
		$qual12 = $qual1 . $qual2;
		if ($realignment eq 'ON'){
		    my $del_pos = $pos + $1 - 1;
		    my $del_info = $chrom . '==' . $del_pos . '==' . $2 . '==D==xxx';
		    $indel_read{$del_info} = 1;
		}
	    }
	    else{
		$ref12 = $ref1;
		...
		if ($MDtag_sam == 0){
		    my ($total_mismatch1, $lowq_mismatch1, $terminal_mismatch_N1, $terminal_mismatch_C1, $MD1) = &count_mismatch_T ($read1, $ref1, $qual1, $chrom, $pos);
		    my ($total_mismatch2, $lowq_mismatch2, $terminal_mismatch_N2, $terminal_mismatch_C2, $MD2) = (0, 0, 0, 0, '');
		    if ($clip_tag == 0){
			my $ref_start2 = $pos + $1 + $2;
	...
	elsif ($cigar =~ /(\d+)M(\d+)([ID])(\d+)M(\d+)([ID])(\d+)M/){
	    my $ref1 = substr ($chr_seq{$chrom}, $pos - 1, $1);
	    my $read1 = substr ($read, 0, $1);
	    my $qual1 = substr ($qual, 0, $1);
	    my $ref2 = '';
	    ...
	    my $clip_tag = 0;
	    my $SC = '';
	    if ((($3 eq 'I') and ($chr_len - $pos - $1 + 1 - $4 >= 0)) or (($3 eq 'D') and ($chr_len - $pos - $1 - $2 + 1 - $4 >= 0))){
		$ref2 = substr ($chr_seq{$chrom}, $pos + $1 - 1, $4) if ($3 eq 'I');
		$ref2 = substr ($chr_seq{$chrom}, $pos + $1 + $2 - 1, $4) if ($3 eq 'D');
		my $ref12 = $ref1 . $ref2;
		$read2 = substr ($read, $1 + $2, $4) if ($3 eq 'I');
		$read2 = substr ($read, $1, $4) if ($3 eq 'D');
		my $read12 = $read1 . $read2;
		$qual2 = substr ($qual, $1 + $2, $4) if ($3 eq 'I');
		$qual2 = substr ($qual, $1, $4) if ($3 eq 'D');
		my $qual12 = $qual1 . $qual2;
		if ((($3 eq 'I') and ($6 eq 'I') and ($chr_len - $pos - $1 + 1 - $4 - $7 >= 0)) or (($3 eq 'I') and ($6 eq 'D') and ($chr_len - $pos - $1 + 1 - $4 - $5 - $7 >= 0))
		    or (($3 eq 'D') and ($6 eq 'I') and ($chr_len - $pos - $1 - $2 + 1 - $4 - $7 >= 0)) or (($3 eq 'D') and ($6 eq 'D') and ($chr_len - $pos - $1 - $2 + 1 - $4 - $5 - $7 >= 0))){   
		    $ref3 = substr ($chr_seq{$chrom}, $pos + $1 + $4 - 1, $7) if (($3 eq 'I') and ($6 eq 'I'));
		    $ref3 = substr ($chr_seq{$chrom}, $pos + $1 + $4 + $5 - 1, $7) if (($3 eq 'I') and ($6 eq 'D'));
		    $ref3 = substr ($chr_seq{$chrom}, $pos + $1 + $2 + $4 - 1, $7) if (($3 eq 'D') and ($6 eq 'I'));
		    $ref3 = substr ($chr_seq{$chrom}, $pos + $1 + $2 + $4 + $5 - 1, $7) if (($3 eq 'D') and ($6 eq 'D'));
		    $ref123 = $ref12 . $ref3;
		    next if (length $ref123 != $1 + $4 + $7);
		    $read3 = substr ($read, $1 + $2 + $4 + $5, $7) if (($3 eq 'I') and ($6 eq 'I'));
		    $read3 = substr ($read, $1 + $2 + $4, $7) if (($3 eq 'I') and ($6 eq 'D'));
		    $read3 = substr ($read, $1 + $4 + $5, $7) if (($3 eq 'D') and ($6 eq 'I'));
		    $read3 = substr ($read, $1 + $4, $7) if (($3 eq 'D') and ($6 eq 'D'));
		    $read123 = $read12 . $read3;
		    $qual3 = substr ($qual, $1 + $2 + $4 + $5, $7) if (($3 eq 'I') and ($6 eq 'I'));
		    $qual3 = substr ($qual, $1 + $2 + $4, $7) if (($3 eq 'I') and ($6 eq 'D'));
		    $qual3 = substr ($qual, $1 + $4 + $5, $7) if (($3 eq 'D') and ($6 eq 'I'));
		    $qual3 = substr ($qual, $1 + $4, $7) if (($3 eq 'D') and ($6 eq 'D'));
		    $qual123 = $qual12 . $qual3;
		    if ($realignment eq 'ON'){
			if ($3 eq 'I'){
			    my $ins_pos = $pos + $1 - 1;
			    my $ins_base = substr ($read, $1, $2);
			    ...
			    $indel_read{$ins_info} = 1;
			    if ($6 eq 'I'){
				my $ins_pos = $pos + $1 + $4 - 1;
				my $ins_base = substr ($read, $1 + $2 + $4, $5);
				my $ins_info = $chrom . '==' . $ins_pos . '==' . $5 . '==I==' . $ins_base;
				$indel_read{$ins_info} = 1;
			    }
			    elsif ($6 eq 'D'){
				my $del_pos = $pos + $1 + $4 - 1;
				my $del_info = $chrom . '==' . $del_pos . '==' . $5 . '==D==xxx';
				$indel_read{$del_info} = 1;
			    }
			}
			elsif ($3 eq 'D'){
			    ...
			    $indel_read{$del_info} = 1;
			    if ($6 eq 'I'){
				my $ins_pos = $pos + $1 + $2 + $4 - 1;
				my $ins_base = substr ($read, $1 + $4, $5);
				my $ins_info = $chrom . '==' . $ins_pos . '==' . $5 . '==I==' . $ins_base;
				$indel_read{$ins_info} = 1;
			    }
			    elsif ($6 eq 'D'){
				my $del_pos = $pos + $1 + $2 + $4 - 1;
				my $del_info = $chrom . '==' . $del_pos . '==' . $5 . '==D==xxx';
				$indel_read{$del_info} = 1;
			    }
			}
		...
		if ($MDtag_sam == 1){
		    ...
		    if ($3 eq 'D'){
			my $ref_del1 = substr ($chr_seq{$chrom}, $pos + $1 - 1, $2);
			if ($6 eq 'D'){
			    my $ref_del2 = substr ($chr_seq{$chrom}, $pos + $1 + $2 + $4 - 1, $5);
			...
			if ($6 eq 'D'){
			    my $ref_del2 = substr ($chr_seq{$chrom}, $pos + $1 + $4 - 1, $5);
		...
		my ($total_mismatch, $lowq_mismatch, $terminal_mismatch_N, $terminal_mismatch_C, $MD) = &count_mismatch_T ($read123, $ref123, $qual123) if ($MDtag_sam == 1);
		if (($MDtag_sam == 0) and ($clip_tag == 0)){
			...
			if ($6 eq 'D'){
			    my $ref_del2 = substr ($chr_seq{$chrom}, $pos + $1 + $4 - 1, $5);
			    my $ref_start3 = $pos + $1 + $2 + $4 + $5;
			    my ($total_mismatch1, $lowq_mismatch1, $terminal_mismatch_N1, $terminal_mismatch_C1, $MD1) = &count_mismatch_T ($read1a, $ref1a, $qual1a, $chrom, $pos);
			    ...
			    ($total_mismatch, $lowq_mismatch, $terminal_mismatch_N, $terminal_mismatch_C, $MD) = &count_mismatch_T ($read1a, $ref1a, $qual1a, $chrom, $pos);
			}
		    }
		}
		my $new_line = $line . "\tmismatch:$total_mismatch\tindel:$indel" if ($MDtag_sam == 1) and ($clip_tag == 0);
	...
	elsif ($cigar =~ /(\d+)M(\d+)([ID])(\d+)M(\d+)([ID])(\d+)M(\d+)([ID])(\d+)M/){
	    my $ref1 = substr ($chr_seq{$chrom}, $pos - 1, $1);
	    my $read1 = substr ($read, 0, $1);
	    my $qual1 = substr ($qual, 0, $1);
	    my $ref2 = '';
		...
		if ((($3 eq 'I') and ($6 eq 'I') and ($chr_len - $pos - $1 + 1 - $4 - $7 >= 0)) or (($3 eq 'I') and ($6 eq 'D') and ($chr_len - $pos - $1 + 1 - $4 - $5 - $7 >= 0))
		    ...
		    $ref3 = substr ($chr_seq{$chrom}, $pos + $1 + $2 + $4 - 1, $7) if (($3 eq 'D') and ($6 eq 'I'));
		    $ref3 = substr ($chr_seq{$chrom}, $pos + $1 + $2 + $4 + $5 - 1, $7) if (($3 eq 'D') and ($6 eq 'D'));
		    my $ref123 = $ref12 . $ref3;
		    next if (length $ref123 != $1 + $4 + $7);
		    $read3 = substr ($read, $1 + $2 + $4 + $5, $7) if (($3 eq 'I') and ($6 eq 'I'));
		    $read3 = substr ($read, $1 + $2 + $4, $7) if (($3 eq 'I') and ($6 eq 'D'));
		    $read3 = substr ($read, $1 + $4 + $5, $7) if (($3 eq 'D') and ($6 eq 'I'));
		    $read3 = substr ($read, $1 + $4, $7) if (($3 eq 'D') and ($6 eq 'D'));
		    my $read123 = $read12 . $read3;
		    $qual3 = substr ($qual, $1 + $2 + $4 + $5, $7) if (($3 eq 'I') and ($6 eq 'I'));
		    $qual3 = substr ($qual, $1 + $2 + $4, $7) if (($3 eq 'I') and ($6 eq 'D'));
		    $qual3 = substr ($qual, $1 + $4 + $5, $7) if (($3 eq 'D') and ($6 eq 'I'));
		    $qual3 = substr ($qual, $1 + $4, $7) if (($3 eq 'D') and ($6 eq 'D'));
		    my $qual123 = $qual12. $qual3;
		    if ((($3 eq 'I') and ($6 eq 'I') and ($9 eq 'I') and ($chr_len - $pos - $1 + 1 - $4 - $7 - $10 >= 0)) or (($3 eq 'I') and ($6 eq 'I') and ($9 eq 'D') and ($chr_len - $pos - $1 + 1 - $4 - $7 - $8 - $10 >= 0))
			or (($3 eq 'I') and ($6 eq 'D') and ($9 eq 'I') and ($chr_len - $pos - $1 + 1 - $4 - $5 - $7 - $10 >= 0)) or (($3 eq 'I') and ($6 eq 'D') and ($9 eq 'D') and ($chr_len - $pos - $1 + 1 - $4 - $5 - $7 - $8 - $10 >= 0)) 
			or (($3 eq 'D') and ($6 eq 'I') and ($9 eq 'I') and ($chr_len - $pos - $1 - $2 + 1 - $4 - $7 - $10 >= 0)) or (($3 eq 'D') and ($6 eq 'I') and ($9 eq 'D') and ($chr_len - $pos - $1 - $2 + 1 - $4 - $7 - $8 - $10 >= 0)) 
			or (($3 eq 'D') and ($6 eq 'D') and ($9 eq 'I') and ($chr_len - $pos - $1 - $2 + 1 - $4 - $5 - $7 - $10 >= 0)) or (($3 eq 'D') and ($6 eq 'D') and ($9 eq 'D') and ($chr_len - $pos - $1 - $2 + 1 - $4 - $5 - $7 - $8 - $10 >= 0))){
			my $pos2 = 0;
			if ($3 eq 'I'){
			    if ($6 eq 'I'){
				$pos2 += $1 + $4 + $7 - 1 if ($9 eq 'I');
				$pos2 += $1 + $4 + $7 + $8 - 1 if ($9 eq 'D');
			    }
			    elsif ($6 eq 'D'){
				$pos2 += $1 + $4 + $5 + $7 - 1 if ($9 eq 'I');
				$pos2 += $1 + $4 + $5 + $7 + $8 - 1 if ($9 eq 'D');
			    }
			}
			elsif ($3 eq 'D'){
			    if ($6 eq 'I'){
				$pos2 += $1 + $2 + $4 + $7 - 1 if ($9 eq 'I');
				$pos2 += $1 + $2 + $4 + $7 + $8 - 1 if ($9 eq 'D');
			    }
			    elsif ($6 eq 'D'){
				$pos2 += $1 + $2 + $4 + $5 + $7 - 1 if ($9 eq 'I');
				$pos2 += $1 + $2 + $4 + $5 + $7 + $8 - 1 if ($9 eq 'D');
			    }
			}
			$ref4 = substr ($chr_seq{$chrom}, $pos + $pos2, $10);
			$ref1234 = $ref123 . $ref4;
			next if (length $ref1234 != $1 + $4 + $7 + $10);
			$read4 = substr ($read, $pos2, $10);
			$read1234 = $read123 . $read4;
			$qual4 = substr ($qual, $pos2, $10);
			$qual1234 = $qual123 . $qual4;
			if ($realignment eq 'ON'){
			    if ($3 eq 'I'){
				my $ins_pos = $pos + $1 - 1;
				my $ins_base = substr ($read, $1, $2);
				...
				if ($6 eq 'I'){
				    ...
				    $indel_read{$ins_info} = 1;
				    if ($9 eq 'I'){
					my $ins_pos = $pos + $1 + $4 + $7 - 1;
					my $ins_base = substr ($read, $1 + $2 + $4 + $5, $8);
					my $ins_info = $chrom . '==' . $ins_pos . '==' . $8 . '==I==' . $ins_base;
					$indel_read{$ins_info} = 1;
				    }
				    elsif ($9 eq 'D'){
					my $del_pos = $pos + $1 + $4 + $7 - 1;
					my $del_info = $chrom . '==' . $del_pos . '==' . $8 . '==D==xxx';
					$indel_read{$del_info} = 1;
				    }
				}
				elsif ($6 eq 'D'){
				    ...
				    $indel_read{$del_info} = 1;
				    if ($9 eq 'I'){
					my $ins_pos = $pos + $1 + $4 + $5 + $7 - 1;
					my $ins_base = substr ($read, $1 + $2 + $4, $8);
					my $ins_info = $chrom . '==' . $ins_pos . '==' . $8 . '==I==' . $ins_base;
					$indel_read{$ins_info} = 1;
				    }
				    elsif ($9 eq 'D'){
					my $del_pos = $pos + $1 + $4 + $5 + $7 - 1;
					my $del_info = $chrom . '==' . $del_pos . '==' . $8 . '==D==xxx';
					$indel_read{$del_info} = 1;
				    }
				}
				...
				if ($6 eq 'I'){
				    ...
				    $indel_read{$ins_info} = 1;
				    if ($9 eq 'I'){
					my $ins_pos = $pos + $1 + $2 + $4 + $7 - 1;
					my $ins_base = substr ($read, $1 + $4 + $5 + $7, $8);
					my $ins_info = $chrom . '==' . $ins_pos . '==' . $8 . '==I==' . $ins_base;
				    }
				    elsif ($9 eq 'D'){
					my $del_pos = $pos + $1 + $2 + $4 + $7 - 1;
					my $del_info = $chrom . '==' . $del_pos . '==' . $8 . '==D==xxx';
					$indel_read{$del_info} = 1;
				    }
				}
				    ...
				    $indel_read{$del_info} = 1;
				    if ($9 eq 'I'){
					my $ins_pos = $pos + $1 + $2 + $4 + $5 + $7 - 1;
					my $ins_base = substr ($read, $1 + $4 + $7, $8);
					my $ins_info = $chrom . '==' . $ins_pos . '==' . $8 . '==I==' . $ins_base;
				    }
				    elsif ($9 eq 'D'){
					my $del_pos = $pos + $1 + $2 + $4 + $5 + $7 - 1;
					my $del_info = $chrom . '==' . $del_pos . '==' . $8 . '==D==xxx';
					$indel_read{$del_info} = 1;
				    }
				}
		...
		my ($total_mismatch, $lowq_mismatch, $terminal_mismatch_N, $terminal_mismatch_C, $MD) = &count_mismatch_T ($read1234, $ref1234, $qual1234) if ($MDtag_sam == 1);
		if (($MDtag_sam == 0) and ($clip_tag == 0)){
		    if ($3 eq 'D'){
			my $ref_del1 = substr ($chr_seq{$chrom}, $pos + $1 - 1, $2);
			my $ref_start2 = $pos + $1 + $2;
			if ($6 eq 'D'){
			    ...
			    my $ref_start3 = $pos + $1 + $2 + $4 + $5;
			    if ($9 eq 'D'){
				my $ref_del3 = substr ($chr_seq{$chrom}, $pos + $1 + $2 + $4 + $5 + $7 - 1, $8);
				my $ref_start4 = $pos + $1 + $2 + $4 + $5 + $7 + $8;
				my ($total_mismatch1, $lowq_mismatch1, $terminal_mismatch_N1, $terminal_mismatch_C1, $MD1) = &count_mismatch_T ($read1, $ref1, $qual1, $chrom, $pos);
				my ($total_mismatch2, $lowq_mismatch2, $terminal_mismatch_N2, $terminal_mismatch_C2, $MD2) = &count_mismatch_T ($read2, $ref2, $qual2, $chrom, $ref_start2);
				my ($total_mismatch3, $lowq_mismatch3, $terminal_mismatch_N3, $terminal_mismatch_C3, $MD3) = &count_mismatch_T ($read3, $ref3, $qual3, $chrom, $ref_start3);
				my ($total_mismatch4, $lowq_mismatch4, $terminal_mismatch_N4, $terminal_mismatch_C4, $MD4) = &count_mismatch_T ($read4, $ref4, $qual4, $chrom, $ref_start4);
				$total_mismatch = $total_mismatch1 + $total_mismatch2 + $total_mismatch3 + $total_mismatch4;
				$lowq_mismatch = $lowq_mismatch1 + $lowq_mismatch2 + $lowq_mismatch3 + $lowq_mismatch4;
				$terminal_mismatch_N = $terminal_mismatch_N1 + $terminal_mismatch_N2 + $terminal_mismatch_N3 + $terminal_mismatch_N4;
				$terminal_mismatch_C = $terminal_mismatch_C1 + $terminal_mismatch_C2 + $terminal_mismatch_C3 + $terminal_mismatch_C4;
				$MD = $MD1 . '^' . $ref_del1 . $MD2 . '^' . $ref_del2 . $MD3 . '^' . $ref_del3 . $MD4;
			    }
			    else{
				my $read3a = $read3 . $read4;
				my $ref3a = $ref3 . $ref4;
				my $qual3a = $qual3 . $qual4;
				my ($total_mismatch1, $lowq_mismatch1, $terminal_mismatch_N1, $terminal_mismatch_C1, $MD1) = &count_mismatch_T ($read1, $ref1, $qual1, $chrom, $pos);
				my ($total_mismatch2, $lowq_mismatch2, $terminal_mismatch_N2, $terminal_mismatch_C2, $MD2) = &count_mismatch_T ($read2, $ref2, $qual2, $chrom, $ref_start2);
				my ($total_mismatch3, $lowq_mismatch3, $terminal_mismatch_N3, $terminal_mismatch_C3, $MD3) = &count_mismatch_T ($read3a, $ref3a, $qual3a, $chrom, $ref_start3);
				$total_mismatch = $total_mismatch1 + $total_mismatch2 + $total_mismatch3;
				$lowq_mismatch = $lowq_mismatch1 + $lowq_mismatch2 + $lowq_mismatch3;
				$terminal_mismatch_N = $terminal_mismatch_N1 + $terminal_mismatch_N2 + $terminal_mismatch_N3;
				$terminal_mismatch_C = $terminal_mismatch_C1 + $terminal_mismatch_C2 + $terminal_mismatch_C3;
			    ...
			    if ($9 eq 'D'){
				my $ref_del3 = substr ($chr_seq{$chrom}, $pos + $1 + $2 + $4 + $7 - 1, $8);
				my $ref_start4 = $pos + $1 + $2 + $4 + $7 + $8;
				my $read2a = $read2 . $read3;
				my $ref2a = $ref2 . $ref3;
				my $qual2a = $qual2 . $qual3;
				my ($total_mismatch1, $lowq_mismatch1, $terminal_mismatch_N1, $terminal_mismatch_C1, $MD1) = &count_mismatch_T ($read1, $ref1, $qual1, $chrom, $pos);
				my ($total_mismatch2, $lowq_mismatch2, $terminal_mismatch_N2, $terminal_mismatch_C2, $MD2) = &count_mismatch_T ($read2a, $ref2a, $qual2a, $chrom, $ref_start2);
				my ($total_mismatch3, $lowq_mismatch3, $terminal_mismatch_N3, $terminal_mismatch_C3, $MD3) = &count_mismatch_T ($read4, $ref4, $qual4, $chrom, $ref_start4);
				$total_mismatch = $total_mismatch1 + $total_mismatch2 + $total_mismatch3;
				$lowq_mismatch = $lowq_mismatch1 + $lowq_mismatch2 + $lowq_mismatch3;
				$terminal_mismatch_N = $terminal_mismatch_N1 + $terminal_mismatch_N2 + $terminal_mismatch_N3;
				$terminal_mismatch_C = $terminal_mismatch_C1 + $terminal_mismatch_C2 + $terminal_mismatch_C3;
				$MD = $MD1 . '^' . $ref_del1 . $MD2 . '^' . $ref_del3 . $MD3;
			    }
			    else{
				my $read2a = $read2 . $read3 . $read4;
				my $ref2a = $ref2 . $ref3 . $ref4;
				my $qual2a = $qual2 . $qual3 . $qual4;
				my ($total_mismatch1, $lowq_mismatch1, $terminal_mismatch_N1, $terminal_mismatch_C1, $MD1) = &count_mismatch_T ($read1, $ref1, $qual1, $chrom, $pos);
				my ($total_mismatch2, $lowq_mismatch2, $terminal_mismatch_N2, $terminal_mismatch_C2, $MD2) = &count_mismatch_T ($read2a, $ref2a, $qual2a, $chrom, $ref_start2);
				$total_mismatch = $total_mismatch1 + $total_mismatch2;
				$lowq_mismatch = $lowq_mismatch1 + $lowq_mismatch2;
			...
			if ($6 eq 'D'){
			    ...
			    my $ref_start3 = $pos + $1 + $2 + $4 + $5;
			    if ($9 eq 'D'){
				my $ref_del3 = substr ($chr_seq{$chrom}, $pos + $1 + $4 + $5 + $7 - 1, $8);
				my $ref_start4 = $pos + $1 + $4 + $5 + $7 + $8;
				my $read1a = $read1 . $read2;
				my $ref1a = $ref1 . $ref2;
				my $qual1a = $qual1 . $qual2;
				my ($total_mismatch1, $lowq_mismatch1, $terminal_mismatch_N1, $terminal_mismatch_C1, $MD1) = &count_mismatch_T ($read1a, $ref1a, $qual1a, $chrom, $pos);
				...
				$terminal_mismatch_N = $terminal_mismatch_N1 + $terminal_mismatch_N2 + $terminal_mismatch_N3;
				$terminal_mismatch_C = $terminal_mismatch_C1 + $terminal_mismatch_C2 + $terminal_mismatch_C3;
				$MD = $MD1 . '^' . $ref_del2 . $MD2 . '^' . $ref_del3 . $MD3;
			    }
			    else{
				my $read1a = $read1 . $read2;
				my $ref1a = $ref1 . $ref2;
				...
				my $qual3a = $qual3 . $qual4;
				my ($total_mismatch1, $lowq_mismatch1, $terminal_mismatch_N1, $terminal_mismatch_C1, $MD1) = &count_mismatch_T ($read1a, $ref1a, $qual1a, $chrom, $pos);
				my ($total_mismatch2, $lowq_mismatch2, $terminal_mismatch_N2, $terminal_mismatch_C2, $MD2) = &count_mismatch_T ($read3a, $ref3a, $qual3a, $chrom, $ref_start3);
				$total_mismatch = $total_mismatch1 + $total_mismatch2;
				$lowq_mismatch = $lowq_mismatch1 + $lowq_mismatch2;
				$terminal_mismatch_N = $terminal_mismatch_N1 + $terminal_mismatch_N2;
				$terminal_mismatch_C = $terminal_mismatch_C1 + $terminal_mismatch_C2;
			    ...
			    if ($9 eq 'D'){
				my $ref_del3 = substr ($chr_seq{$chrom}, $pos + $1 + $4 + $7 - 1, $8);
				my $ref_start4 = $pos + $1 + $4 + $7 + $8;
				my $read1a = $read1 . $read2 . $read3;
				my $ref1a = $ref1 . $ref2 . $ref3;
				my $qual1a = $qual1 . $qual2 . $qual3;
				my ($total_mismatch1, $lowq_mismatch1, $terminal_mismatch_N1, $terminal_mismatch_C1, $MD1) = &count_mismatch_T ($read1a, $ref1a, $qual1a, $chrom, $pos);
				my ($total_mismatch2, $lowq_mismatch2, $terminal_mismatch_N2, $terminal_mismatch_C2, $MD2) = &count_mismatch_T ($read4, $ref4, $qual4, $chrom, $ref_start4);
				$total_mismatch = $total_mismatch1 + $total_mismatch2;
				$lowq_mismatch = $lowq_mismatch1 + $lowq_mismatch2;
				$terminal_mismatch_N = $terminal_mismatch_N1 + $terminal_mismatch_N2;
				$terminal_mismatch_C = $terminal_mismatch_C1 + $terminal_mismatch_C2;
	    ...
	    my $chr_pos = $chrom . '==' . $pos09d;
	    my $chr_pos_id = $chrom . '==' . $pos09d . '==' . $read_id . '==' . $direct;
	    my $ref1 = substr ($chr_seq{$chrom}, $pos - 1, $2);
	    my $read1 = substr ($read, $1, $2);
	    my $qual1 = substr ($qual, $1, $2);
	    my $SC = $1;
	    if ($read1 eq $ref1){
		...
		my ($total_mismatch, $lowq_mismatch, $terminal_mismatch_N, $terminal_mismatch_C, $MD) = &count_mismatch_T ($read1, $ref1, $qual1, $chrom, $pos);
	    ...
	    my $read1 = substr ($read, $1, $2);
	    my $qual1 = substr ($qual, $1, $2);
	    my $ref2 = substr ($chr_seq{$chrom}, $pos + $2 + $3 - 1, $5) if ($4 eq 'D');
	    $ref2 = substr ($chr_seq{$chrom}, $pos + $2 - 1, $5) if ($4 eq 'I');
	    my $read2 = substr ($read, $1 + $2, $5) if ($4 eq 'D');
	    $read2 = substr ($read, $1 + $2 + $3, $5) if ($4 eq 'I');
	    my $qual2 = substr ($qual, $1 + $2, $5) if ($4 eq 'D');
	    $qual2 = substr ($qual, $1 + $2 + $3, $5) if ($4 eq 'I');
	    my $ref12 = $ref1 . $ref2;
	    my $read12 = $read1 . $read2;
	    my $qual12 = $qual1 . $qual2;
	    my $SC = $1;
		...
		if ($4 eq 'I'){
		    my $ins_pos = $pos + $1 - 1;
		    my $ins_base = substr ($read, $1 + $2, $3);
		    my $ins_info = $chrom . '==' . $ins_pos . '==' . $3 . '==I==' . $ins_base;
		    $indel_read{$ins_info} = 1;
		}
		elsif ($4 eq 'D'){
		    my $del_pos = $pos + $1 - 1;
		    my $del_info = $chrom . '==' . $del_pos . '==' . $3 . '==D==xxx';
		    $indel_read{$del_info} = 1;
		}
	    }
	    if ($read12 eq $ref12){
		my $new_line = $line . "\tmismatch:0\tindel:$indel\tsoftclip:$SC" if ($MDtag_sam == 1);
		$new_line = $line . "\tMD:Z:$2\tmismatch:0\tindel:$indel\tsoftclip:$SC" if ($MDtag_sam == 0);
		if ($realignment eq 'ON'){
		    unless (exists $mismatch_read{$chr_pos}){
			$mismatch_read{$chr_pos} = $new_line;
		    }
		...
		my ($total_mismatch, $lowq_mismatch, $terminal_mismatch_N, $terminal_mismatch_C, $MD) = &count_mismatch_T ($read12, $ref12, $qual12, $chrom, $pos);
		my $new_line = $line . "\tmismatch:$total_mismatch\tindel:$indel\tsoftclip:$SC" if ($MDtag_sam == 1);
		$new_line = $line . "\tMD:Z:$MD\tmismatch:$total_mismatch\tindel:$indel\tsoftclip:$SC" if ($MDtag_sam == 0);
		if ($read_correct == 1){
		    if ($lowq_mismatch > $max_mismatch_num){
			$filtered_pairs{$read_id} = 1 if ($filter_pairs == 1);
		    }
	...
	elsif ($cigar =~ /^(\d+)M(\d+)([ID])(\d+)M(\d+)S$/){
	    my $ref1 = substr ($chr_seq{$chrom}, $pos - 1, $1);
	    my $read1 = substr ($read, 0, $1);
	    my $qual1 = substr ($qual, 0, $1);
	    my $ref2 = substr ($chr_seq{$chrom}, $pos + $1 + $2 - 1, $4) if ($3 eq 'D');
	    $ref2 = substr ($chr_seq{$chrom}, $pos + $1 - 1, $4) if ($3 eq 'I');
	    my $read2 = substr ($read, $1, $4) if ($3 eq 'D');
	    $read2 = substr ($read, $1 + $2, $4) if ($3 eq 'I');
	    my $qual2 = substr ($qual, $1, $4) if ($3 eq 'D');
	    $qual2 = substr ($qual, $1 + $2, $4) if ($3 eq 'I');
	    my $ref12 = $ref1 . $ref2;
	    my $read12 = $read1 . $read2;
	    my $qual12 = $qual1 . $qual2;
	    ...
	    if ($read12 eq $ref12){
		my $new_line = $line . "\tmismatch:0\tindel:$indel\tsoftclip:$SC" if ($MDtag_sam == 1);
		$new_line = $line . "\tMD:Z:$1\tmismatch:0\tindel:$indel\tsoftclip:$SC" if ($MDtag_sam == 0);
		if ($realignment eq 'ON'){
		    unless (exists $mismatch_read{$chr_pos}){
			$mismatch_read{$chr_pos} = $new_line;
		    }
	    ...
	    my $read12 = $read;
	    my $qual12 = $qual;
	    if ($cigar =~ /^(\d+)M(\d+)N(\d+)M$/){
		my $ref1 = substr ($chr_seq{$chrom}, $pos - 1, $1);
		my $ref2 = substr ($chr_seq{$chrom}, $pos + $1 + $2 - 1, $3);
	    }
	    elsif ($cigar =~ /^(\d+)M(\d+)N(\d+)M(\d+)N(\d+)M$/){
		my $ref1 = substr ($chr_seq{$chrom}, $pos - 1, $1);
		my $ref2 = substr ($chr_seq{$chrom}, $pos + $1 + $2 - 1, $3);
		my $ref3 = substr ($chr_seq{$chrom}, $pos + $1 + $2 + $3 + $4 - 1, $5);
...
sub realign_indel{      
	...
	for (my $posi = $indelpos - $ave_read_length + 2; $posi <= $indelpos + $del_size; $posi++){
		...
		foreach my $mismatch_line (@mismatch_read_lines){
		    ...
		    my $ref3 = '';
		    my $mismatch_count = 0;
		    if ($cigar =~ /^(\d+)M(\d+)([ID])(\d+)M(\d+)([ID])(\d+)M/){
			if ($indel eq 'I'){
			    if ($indelpos - $posi + 1 < $read_length / 2){
				next if ($indelpos - $posi + 1 >= $1) or ($1 >= $read_length / 2);
			    }
			    else{
				my $posi_last = 0;
				if (($3 eq 'I') and ($6 eq 'I')){
				    $posi_last = $posi + $1 + $4;
				}
				elsif (($3 eq 'I') and ($6 eq 'D')){
				    $posi_last = $posi + $1 + $4 + $5;
				}
				elsif (($3 eq 'D') and ($6 eq 'I')){
				    $posi_last = $posi + $1 + $2 + $4;
				}
				elsif (($3 eq 'D') and ($6 eq 'D')){
				    $posi_last = $posi + $1 + $2 + $4 + $5;
				}
				next if ($indelpos - $posi_last + 1 >= $7) or ($7 >= $read_length / 2);
			    }
			}
			elsif ($indel eq 'D'){
			    if (($indelpos <= $posi) or ($indelpos - $posi + 1 <= $read_length - $indelsize - $indelpos + $posi - 1)){
				next if ($indelpos - $posi + 1 >= $1) or ($1 >= ($read_length - $indelsize) / 2);
			    }
			    else{
				my $posi_last = 0;
				if (($3 eq 'I') and ($6 eq 'I')){
				...
				next if ($indelpos - $posi_last + 1 >= $7) or ($7 >= ($read_length - $indelsize) / 2);
			    }
			}
		    }
		    elsif ($cigar =~ /^(\d+)M(\d+)([ID])(\d+)M(\d+)S$/){
			if ($indel eq 'I'){
				...
				next if ($indelpos - $posi_last + 1 >= $4) or ($4 + $5 >= $read_length / 2);
			    }
			}
			elsif ($indel eq 'D'){
			    if (($indelpos <= $posi) or ($indelpos - $posi + 1 <= $read_length - $indelsize - $indelpos + $posi - 1)){
				...
				next if ($indelpos - $posi_last + 1 >= $4) or ($4 + $5 >= ($read_length - $indelsize) / 2);
			    }
			}
		    }
		    elsif ($cigar =~ /^(\d+)S(\d+)M(\d+)([ID])(\d+)M$/){
			if ($indel eq 'I'){
			    if ($indelpos - $posi + 1 + $1 < $read_length / 2){
				next if ($indelpos - $posi + 1 >= $2) or ($1 + $2 >= $read_length / 2);
			    }
			    else{
				my $posi_last = 0;
				if ($4 eq 'I'){
				...
				next if ($indelpos - $posi_last + 1 >= $5) or ($5 >= $read_length / 2);
			    }
			}
			elsif ($indel eq 'D'){
			    if (($indelpos <= $posi) or ($indelpos - $posi + 1 <= $read_length - $indelsize - $indelpos + $posi - 1)){
				next if ($indelpos - $posi + 1 >= $2) or ($1 + $2 >= ($read_length - $indelsize) / 2);
			    }
			    else{
				my $posi_last = 0;
				if ($4 eq 'I'){
				...
				next if ($indelpos - $posi_last + 1 >= $5) or ($5 >= ($read_length - $indelsize) / 2);
			    }
			}
		    }
		    
		    if (($cigar eq $read_length . 'M') or ($cigar =~ /^\d+M\d+[ID]\d+M\d+[ID]\d+M/) or ($cigar =~ /^\d+M\d+[ID]\d+M\d+S$/) or ($cigar =~ /^\d+S\d+M\d+[ID]\d+M$/)){
			if ($indel eq 'I'){
			    if (($indelpos - $posi + 1) < $read_length / 2){
				if (($indelpos - $posi + 1) > $indelsize){
				    my $ins_read = substr ($read, $indelpos - $posi + 1 - $indelsize, $indelsize);
				    next if ($ins_read ne $insbase);
				    $ref12 = substr ($chr_seq{$chr}, $posi - 1 + $indelsize, $read_length - $indelsize);
				    $read1 = substr ($read, 0, $indelpos - $posi + 1 - $indelsize);
				    next if ($indelpos - $posi + 1 > $read_length - 1);
				    $read2 = substr ($read, $indelpos - $posi + 1);
				    $read12 = $read1 . $read2;
				    $line[3] += $indelsize;
				    ...
				    my $ins_read = substr ($read, 0, $indelpos - $posi + 1);
				    ...
				    $read12 = substr ($read, $indelpos - $posi + 1);
				...
				if ($read_length - $indelpos + $posi - 1 > $indelsize){
				    my $ins_read = substr ($read, $indelpos - $posi + 1, $indelsize);
				    next if ($ins_read ne $insbase);
				    $ref12 = substr ($chr_seq{$chr}, $posi - 1, $read_length - $indelsize);
				    $read1 = substr ($read, 0, $indelpos - $posi + 1);
				    next if ($indelpos - $posi + 1 + $indelsize > $read_length - 1);
				    $read2 = substr ($read, $indelpos - $posi + 1 + $indelsize);
				    $read12 = $read1 . $read2;
				    next if (length $read12 != length $ref12);
				    if ($read12 eq $ref12){
				    ...
				    my $ins_read = substr ($read, $indelpos - $posi + 1, $read_length - $indelpos + $posi - 1);
				    next if ($ins_read ne substr ($insbase, 0, length ($ins_read)));
				    $ref12 = substr ($chr_seq{$chr}, $posi - 1, $indelpos - $posi + 1);
				    $read12 = substr ($read, 0, $indelpos - $posi + 1);
				    next if (length $read12 != length $ref12);
				    if ($read12 eq $ref12){
			    ...
			    if (($indelpos <= $posi) or ($indelpos - $posi + 1 <= $read_length - $indelsize - $indelpos + $posi - 1)){
				next if (length $chr_seq{$chr} < $read_length + $posi - 1);
				...
				next if (length $chr_seq{$chr} < $posi + $indelsize + $read_length- 1);
			...
			if ($indel eq 'I'){
				...
				if ($posi + $1 - 1 > $indelpos){
					...
					$read1 = substr ($read, 0, $indelpos - $posi + 1 - $indelsize);
					next if ($indelpos - $posi + 1 + $indelsize > $read_length - 1);
					$read2 = substr ($read, $indelpos - $posi + 1, $1 - $indelsize - length $read1);
					$read3 = substr ($read, $1 + $2, $3);
					...
					my $ins_read = substr ($read, 0, $indelpos - $posi + 1);
					next if ($ins_read ne substr ($insbase, length ($insbase) - length ($ins_read)));
					$read1 = substr ($read, $indelpos - $posi + 1, $1 - $indelpos + $posi - 1);
					next if ($indelpos - $posi + 1 + $indelsize > $read_length - 1);
					$read2 = substr ($read, $1 + $2, $3);
					$read12 = $read1 . $read2;
					...
					my $ins_read = substr ($read, $indelpos - $posi + 1 - $indelsize + $2, $indelsize);
					next if ($ins_read ne $insbase);
					$ref12 = substr ($chr_seq{$chr}, $posi - 1 - $indelsize - $2, $read_length - $indelsize);
					$read1 = substr ($read, 0, $indelpos - $posi + 1 - $indelsize + $2);
					next if ($indelpos - $posi + 1 + $indelsize > $read_length - 1);
					$read2 = substr ($read, $indelpos - $posi + 1 + $2);
					$read12 = $read1 . $read2;
					...
					my $ins_read = substr ($read, 0, $indelpos - $posi + 1 + $2);
					next if ($ins_read ne substr ($insbase, length ($insbase) - length ($ins_read)));
				...
				if ($posi + $1 - 1 >= $indelpos){	# insersion in read is neglected and the read is treaed as a normal read
					...
					my $ins_read = substr ($read, $2 + $indelpos - $posi + 1, $indelsize);
					next if ($ins_read ne $insbase);
					$ref12 = substr ($chr_seq{$chr}, $posi - 1, $read_length - $indelsize - $2);
					$read1 = substr ($read, 0, $1);
					$read2 = substr ($read, $1 + $2, $indelpos - $posi + 1 - $1);
					next if (length ($read1) + length ($read2) + $2 + $indelsize > $read_length - 1);
					$read3 = substr ($read, length ($read1) + length ($read2) + $2 + $indelsize);
					$read12 = $read1 . $read2 . $read3;
					next if (length $read12 != length $ref12);
					if ($read12 eq $ref12){
					...
					my $ins_read = substr ($read, $2 + $indelpos - $posi + 1, $read_length - $indelpos + $posi - 1 - $2);
					next if ($ins_read ne substr ($insbase, 0, length ($ins_read)));
					$read1 = substr ($read, 0, $1);
					$read2 = substr ($read, $1 + $2, $indelpos - $posi + 1 - $1);
					$read12 = $read1 . $read2;
				...
				if ($posi + $1 - 1 > $indelpos + $indelsize){
				    next if (length $chr_seq{$chr} < $read_length + $posi - 1 - $2);
				    $ref1 = substr ($chr_seq{$chr}, $posi - 1 - $indelsize, $indelpos - $posi + 1 + $indelsize);
				...
				if ($posi + $1 - 1 >= $indelpos){	# insersion in read is neglected and the read is treaed as a normal read
				    next if (length $chr_seq{$chr} < $read_length + $indelsize + $posi - 1);
				    $ref1 = substr ($chr_seq{$chr}, $posi - 1, $indelpos - $posi + 1);
				    $ref2 = substr ($chr_seq{$chr}, $indelpos + $indelsize, $read_length - length $ref1);
				    $ref12 = $ref1 . $ref2;
				    $read12 = $read;
				    ...
				    next if (length $chr_seq{$chr} < $read_length + $indelsize - $2 + $posi - 1);
				    $ref1 = substr ($chr_seq{$chr}, $posi - 1, $indelpos - $posi + 1);
			...
			if ($indel eq 'I'){
				...
				if ($posi + $1 - 1 > $indelpos){
					...
					my $ins_read = substr ($read, 0, $indelpos - $posi + 1 - $2);
					next if ($ins_read ne substr ($insbase, length ($insbase) - length ($ins_read)));
					$read12 = substr ($read, $indelpos - $posi + 1 - $2);
					$ref12 = substr ($chr_seq{$chr}, $indelpos, length $read12);
					my $indelsize_term = $indelpos - $posi + 1;
					$line[3] = $indelpos + 1;
					next if (length $read12 != length $ref12);
				...
				if ($posi + $1 - 1 + $2 >= $indelpos){	# deletion in read is neglected and the read is treaed as a normal read
				    ...
				    if ($read_length - $indelpos + $posi - 1 + $2 > $indelsize){
					my $ins_read = substr ($read, $indelpos - $posi + 1 - $2, $indelsize);
					next if ($ins_read ne $insbase);
					$ref1 = substr ($chr_seq{$chr}, $posi - 1, $1);
					$ref2 = substr ($chr_seq{$chr}, $posi - 1 + $1 + $2, $3 - $indelsize);
					$ref12 = $ref1 . $ref2;
					$read1 = substr ($read, 0, $indelpos - $posi + 1 - $2);
					next if ($indelpos - $posi + 1 - $2 + $indelsize > $read_length - 1);
					$read2 = substr ($read, $indelpos - $posi + 1 - $2 + $indelsize);
					$read12 = $read1 . $read2;
					next if (length $read12 != length $ref12);
					if ($read12 eq $ref12){
					...
					my $ins_read = substr ($read, $indelpos - $posi + 1 - $2, $read_length - $indelpos + $posi - 1  + $2);
					next if ($ins_read ne substr ($insbase, 0, length ($ins_read)));
					$ref1 = substr ($chr_seq{$chr}, $posi - 1, $1);
					$ref2 = substr ($chr_seq{$chr}, $posi - 1 + $1 + $2, $indelpos - $posi + 1 - $2 - $1);
					$ref12 = $ref1 . $ref2;
					$read12 = substr ($read, 0, $indelpos - $posi + 1 - $2);
					next if (length $read12 != length $ref12);
					if ($read12 eq $ref12){
			    ...
			    if (($indelpos <= $posi) or ($indelpos - $posi + 1 <= $read_length - $indelsize - $indelpos + $posi - 1)){
				if ($posi + $1 - 1 > $indelpos + $indelsize){
				    next if (length $chr_seq{$chr} < $1 + $posi - 1);
				    $ref1 = substr ($chr_seq{$chr}, $posi - 1 - $indelsize, $indelpos - $posi + 1 + $indelsize);
				...
				if ($posi + $1 - 1 + $2 >= $indelpos){	# deletion in read is neglected and the read is treaed as a normal read
				    ...
				    next if (length $chr_seq{$chr} < $read_length + $indelsize + $posi - 1 + $2);
				    $ref1 = substr ($chr_seq{$chr}, $posi - 1, $1);
		    ...
		    elsif (($cigar =~ /^(\d+)S(\d+)M$/) or ($cigar =~ /^(\d+)M(\d+)S$/)){
			...
			if ($indel eq 'I'){
				...
				if (($indelpos - $posi + 1) > $indelsize){
				    ...
				    $shift_pos = $indelsize - $SC_num if ($cigar =~ /^\d+S\d+M$/);
				    $shift_pos = $indelsize if ($cigar =~ /^\d+M\d+S$/);
				    next if (length $read12 != length $ref12);
				    if ($read12 eq $ref12){
					$line[5] = ($indelpos - $posi + 1 - $indelsize) . 'M' . $indelsize . 'I' . ($read_length - $indelpos + $posi - 1) . 'M';
					my $new_line = join ("\t", @line);
				    ...
				    $read12 = substr ($read, $indelpos - $posi + 1);
				    $line[3] = $indelpos + 1;
				    $shift_pos = $indelpos - $posi + 1 - $SC_num if ($cigar =~ /^\d+S\d+M$/);
				    $shift_pos = $indelpos - $posi + 1 if ($cigar =~ /^\d+M\d+S$/);
				    next if (length $read12 != length $ref12);
				    if ($read12 eq $ref12){
					$line[5] = ($indelpos - $posi + 1) . 'I' . ($read_length - $indelpos + $posi - 1) . 'M';
					my $new_line = join ("\t", @line);
				...
				if ($read_length - $indelpos + $posi - 1 > $indelsize){
				    ...
				    $read12 = $read1 . $read2;
				    $line[3] = $posi;
				    $shift_pos = $SC_num if ($cigar =~ /^(\d+)S(\d+)M$/);
				    $shift_pos = 0 if ($cigar =~ /^(\d+)M(\d+)S$/);
				    next if (length $read12 != length $ref12);
				    if ($read12 eq $ref12){
					$line[5] = ($indelpos - $posi + 1) . 'M' . $indelsize . 'I' . ($read_length - $indelpos + $posi- 1 - $indelsize) . 'M';
					my $new_line = join ("\t", @line);
				    ...
				    my $ins_read = substr ($read, $indelpos - $posi + 1);
				    next if ($ins_read ne substr ($insbase, 0, length ($ins_read)));
				    $ref12 = substr ($chr_seq{$chr}, $posi - 1, $indelpos - $posi + 1);
				    $read12 = substr ($read, 0, $indelpos - $posi + 1);
				    $line[3] = $posi;
